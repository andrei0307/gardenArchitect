...
if(isset($_POST["type"]) && $_POST["type"]=='add' && isset($_SESSION["user"]))
{
    ...
    $statement->execute();
    $statement->bind_result($id,$product_name,$type,$product_image, $product_price);
...
if(isset($_POST["product_qty"]) || isset($_POST["remove_code"]))
{
	...
	if(isset($_POST["product_qty"]) && is_array($_POST["product_qty"])){
		foreach($_POST["product_qty"] as $key => $value){
			if(is_numeric($value)){
				echo $key;
				$_SESSION["cart_products"][$key]["quantity"] = $value;
			}
		}
	}
